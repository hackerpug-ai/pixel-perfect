...
The preplanning phase creates `{epic}/design/design.config.yaml`:
...
# Target platforms (from multi-select)
platforms:
  - mobile-ios
  - mobile-android
  - web-desktop
...
# Analyzed references (if URLs found)
references:
  urls_analyzed: 2
  patterns:
    - "card-based layout"
...
To reconfigure, run `/pixel-perfect:init --force` or `/pixel-perfect:design --reconfigure`.
