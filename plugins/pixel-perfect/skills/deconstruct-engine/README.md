...
```
LOAD_SKILL("deconstruct-engine", "design/concepts/<name>.html --output design/system")
```
...
2. Run `pixel-perfect:design-deconstruct concept.html` to build the initial system
3. Design another page or iterate on the same one, export again
4. Run `pixel-perfect:design-deconstruct updated-concept.html` — the skill detects existing output and merges
...
pixel-perfect:design-deconstruct <concept-html-path> --doctrine <constraints.md>
pixel-perfect:design-deconstruct <concept-html-path> --parallel <N>
pixel-perfect:design-deconstruct <concept-html-path> --fidelity
pixel-perfect:design-deconstruct <concept-html-path> --resume-from <phase>
...
Failures rerun `DESIGN_EXECUTE` for the offending component with the violating lines quoted.
...
2. Runs targeted `DESIGN_EXECUTE` to extend the lower-layer component (re-author its `_src.html` additively)
...
- **Bundled design contract** — `../../docs/DESIGN-CONTRACT.md`; `frontend-designer` executes it when available and the primary agent executes it directly otherwise
