...
3. **Confirm the screen list with the user** before generating (as `init` and `build` do):
   ...
   - header: Screens
     question: Wireframe these three screens?
     options:
       - label: Wireframe all three (Recommended)
         description: Generates one low-fidelity wireframe per screen, each annotated with its regions, states, and the components it implies. One wireframe becomes one route when init reads them.
       - label: Change the list
         description: Choose Other and name the screens to add, remove, or rename. Nothing is generated until the list is right, because each wireframe becomes a route the whole build is organized around.
   ```
...
For **each screen, sequentially**, call the design primitive with a low-fidelity override brief:

```
DESIGN_EXECUTE(<ASCII WIREFRAME CONTRACT below>
...
- Do NOT scan for components or write product code.
- No color, no fonts, no pixel values. Fidelity is STRUCTURAL: layout, IA, hierarchy, labels, states.
- Box-drawing vocabulary + composition: use the vocabulary below; no external design skill is required
