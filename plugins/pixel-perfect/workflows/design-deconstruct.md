...
| **Image** | View the screenshot and run `DESIGN_EXECUTE` to reconstruct a faithful concept HTML from the visual (layout, hierarchy, colors, type). |
...
| **Concept** (text/PRD) | Run `DESIGN_EXECUTE` to synthesize a distinctive, coherent concept HTML from the description. |
...
2. **Native sandbox.** Generate the component browser (registry + two-pane navigator + run command + **capture command**, sandbox-spec piece #8) per `custom-sandbox.md`. (Use Storybook only if `tools.sandbox` is `storybook*`.)
3. **Resolve the styling contract.** The mockups in `design/system/` carry `<style>` blocks — those are a **pixel reference, never the deliverable**. Before building any native component, resolve the styling contract for `(platform, framework, style, components)` exactly as `pixel-perfect:init` EQUIP does: built-in match (`docs/styling-contracts/`) → else `pixel-perfect:research --styling` → else fail closed. Record `tools.style_contract` + `style_contract_source` in the manifest. (If the manifest already has a resolved contract from a prior init, reuse it.)
4. **Build the layers, gated — translating styles, never porting them.** In order — **atoms → molecules → organisms → views** — build each item as a **real component in the target framework**, register it in the sandbox under its layer, and add a `// Target: design/system/{layer}/{name}/dark.png` reference. Match the mockup's structure / tokens / states. **Translate every style from the mockup's `<style>` block into the styling contract's emit method** (utility classes / `StyleSheet.create` / view modifiers / etc.) — do **not** extract the mockup CSS verbatim into a `styles/` folder of global `.atom-*` / `.mol-*` classes. That parallel-system drift is exactly what the contract exists to prevent (it was the `fabrio` failure). Honor the layer gates — including the styling-contract gate (`scripts/verify-styling-contract.mjs`, `hard-fail` blocks) **and the catalog-capture gate** (`scripts/verify-catalog.mjs --baseline`, exit 0) — so a layer must render in the sandbox, pass the contract gate, **and write goldens** before the next (and the theme/color confirm gate before atoms). Pause for the same human checkpoints `build` uses.
5. **Pixel-targets stay.** The HTML/PNG in `design/system/` remain the design spec; the target-framework code is the deliverable. The mockup CSS is never shipped.
5b. **Catalog goldens per layer.** After each layer is approved, run `node {plugin}/scripts/verify-catalog.mjs --baseline <project-root>` so a deconstruct-seeded project arrives at later `build`/`verify` already fingerprinted under `design/goldens/{platform}/…`.
6. **Record in the manifest.** Write/update `design/manifest.json`: set `tools.framework` + `tools.sandbox: "custom"` + `tools.style_contract` + `tools.style_contract_source` + `capture` (`command`, `medium`, `goldens`, `captured_at`), the built `atoms`/`molecules`/`screens` inventory (status `verified`, each with its `target` — **no** authored composition-edge arrays, **no** `controls` authority field), and mark the matching gates built (including `{layer}_capture`) — so a later `status`/`build` sees a built native project, not a fresh one.
