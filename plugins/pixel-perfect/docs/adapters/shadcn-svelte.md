...
> **Pull-all during scaffold:** when `tools.components` is `shadcn-svelte`, the scaffold workflow pulls every component up front with `npx shadcn-svelte@latest add --all --overwrite` and generates a story per component (mirrors the shadcn/React flow — see `workflows/scaffold.md` Step 2b).
