    ...
    "snapshots": {
      ...
      "items": {
        ...
        "properties": {
          "id":         { "type": "string" },
          "file":       { "type": "string" },
          "layer":      { "enum": ["foundations", "atoms", "molecules", "organisms", "templates", "pages", "use-cases", "views"] },
          "theme":      { "enum": ["dark", "light"], "description": "one theme per snapshot (THEME-PAIR contract)" },
          "state":      { "type": "string", "description": "for state-split views: the route→state path, e.g. rig/arena-terminal/timeout (omit for components and single-state routes)" },
          "mobile":     { "type": "boolean", "description": "true for the 375px mobile render (views only); desktop renders omit or set false" },
...
## ReturnEnvelope — subagent → orchestrator contract

Every phase subagent's final message is ONE JSON object with this shape. On
harnesses with structured output (Claude Code Agent/Workflow schema) it is
enforced at the tool-call layer; elsewhere the orchestrator parses the last
JSON object in the reply, falling back to the legacy string protocol
(`TOKEN_GAP: …` / `VARIANT_REQUEST: …` lines at the top of the reply).
...
```json
{
  ...
  "type": "object",
  "required": ["status", "component", "files_written", "escapes"],
  "additionalProperties": false,
  "properties": {
    "status": { "enum": ["complete", "blocked"] },
    "component": {
      "type": "string",
      "description": "layer/name, e.g. \"atoms/button\" or \"views/street/no-fit\""
    },
    "files_written": {
      "type": "array",
      "items": { "type": "string" },
      "description": "output-relative paths this dispatch wrote"
    },
    "escapes": {
      "type": "array",
      "items": {
        "oneOf": [
          {
            "type": "object",
            "required": ["type", "key", "value", "reason"],
            "additionalProperties": false,
            "properties": {
              "type": { "const": "TOKEN_GAP" },
          ...
          {
            "type": "object",
            "required": ["type", "layer", "name", "variant", "recipe", "reason"],
            "additionalProperties": false,
            "properties": {
              "type": { "const": "VARIANT_REQUEST" },
              "layer": { "enum": ["atom", "molecule", "organism"] },
              "name": { "type": "string" },
              "variant": { "type": "string" },
              "recipe": { "type": "string" },
              "reason": { "type": "string" }
...
- **Escapes are requests, not permissions** — the subagent lists them AND still
  authors its primary deliverable as far as possible.
- **`blocked`** means the component is genuinely unauthorable (unreadable
  concept region, contradictory doctrine). A missing token/variant is never
  `blocked` — it is an escape.
- The orchestrator resolves escapes deterministically: TOKEN_GAP → edit
  `build-tokens.mjs` tables + rebuild; VARIANT_REQUEST → the Regen Handler
  (PHASE-CONTRACTS.md). Under `--parallel`, escapes queue at the wave barrier
  and are deduped by `key` / `(layer,name,variant)` before resolution.

## manifest.json § coverage — the --views-from claim map

When `--views-from <inventory>` is active, Phase 5 records one claim per
inventory variant as it dispatches (or defers) each mock. `_coverage.mjs`
joins inventory × claims and VERIFIES every "mocked" path on disk — a claim
naming a folder without the theme-pair artifacts counts as broken, so a claim
is never accepted as coverage by itself.

```json
{
  "coverage": {
    "type": "object",
    "description": "key = route::state::n from the inventory file",
    "additionalProperties": {
      "oneOf": [
        {
          "type": "object",
          ...
          "properties": {
            "status": { "const": "mocked" },
            "path": { "type": "string", "description": "views/… leaf folder holding dark/light.{html,png}" }
          }
        },
        {
          "type": "object",
