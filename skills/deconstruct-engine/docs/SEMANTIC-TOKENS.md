...
└──────────────────────┘
           ...
           ▼
┌──────────────────────┐
│  tokens.css          │  /* Tier 1 — concept primitives, --_ prefixed */
│  (two tiers, LIVE    │  :root,[data-theme="dark"]  { --_accent: #c85541; }
│   round-trip)        │  [data-theme="light"]       { --_accent: #8c3a28; }
│                      │  /* Tier 2 — semantic alias REFERENCES Tier 1 via var() */
│                      │  :root,[data-theme] { --accent-primary: var(--_accent); }
└──────────────────────┘
```

Components read Tier-2 CSS vars (`var(--accent-primary)`). Tier 2 resolves through
`var(--_accent)` to the Tier-1 primitive — so the round-trip is **live** (the source
value is literally in the CSS, not baked into the alias). A theme flips Tier 1; Tier 2
re-resolves. Nothing in any component changes.

**Why `:root, [data-theme]` (not `[data-theme="dark"]` overrides):** Tier 2 is declared
under BOTH `:root` and `[data-theme]` so its `var()` substitution RE-RESOLVES inside any
themed subtree. If Tier 2 were declared only on `:root`, the aliases would compute once
against `:root`'s primitives and inherit as resolved values — a nested
`[data-theme="light"]` would NOT switch. Matching `[data-theme]` re-declares the whole
semantic layer on the themed element, re-resolving against its `--_` scope.
...
### Step 4 — Generator emits tokens.css (live two-tier round-trip)

The deterministic generator `tokens/build-tokens.mjs` emits from the CONCEPT + SEMANTIC
data tables (the Phase-1 subagent fills those; the generator owns the emit). Tier 1
holds the source primitives `--_`-prefixed; Tier 2 aliases REFERENCE them via `var()`:

```css
/* GENERATED by tokens/build-tokens.mjs — DO NOT EDIT BY HAND */

/* Tier 1 — concept primitives (the source's own keys, --_ prefixed) */
:root, [data-theme="dark"] {           /* dark = EXACT source values */
  --_bg-page: #151513;
  --_accent:  #c85541;
}
[data-theme="light"] {                 /* light = source value, or DERIVED (flagged) */
  --_bg-page: #f7f6f3;
  --_accent:  #8c3a28;
}

/* Tier 2 — semantic aliases reference Tier 1 (declared under :root AND [data-theme]) */
:root, [data-theme] {
  --surface-page:   var(--_bg-page);
  --accent-primary: var(--_accent);
}
```

The alias never bakes a hex — it points at the primitive — so the round-trip is live and
human-inspectable, and a theme flip needs no component edits.

### Round-trip + derived guarantees

- **Round-trip:** every non-derived Tier-2 token's **dark** value equals the source value
  exactly (Tier 2 → `var(--_x)` → Tier-1 primitive = source). The generator fails closed on
  dangling refs, duplicate keys, or a Tier-2 cssVar colliding with a Tier-1 `--_` name.
- **Derived theme:** when the source is **single-theme** (e.g. dark-only), the other theme's
  values are *derived*, not in the source — flagged `derived: true` in `semantic-tokens.json`
  and ⚠ in `TOKEN-MAP.md`. Never silently presented as source.
- **`TOKEN-MAP.md`** (emitted): the human bidirectional map — forward (semantic → concept key
  → Tier-1 var → dark/light value) and reverse (concept key → which semantic roles back it,
  with *unpromoted* keys listed). Lets a reader trace any value both directions without
  opening JSON.
