...
**How it's produced:** the subagent authors ONE document with both theme panes; the orchestrator runs `node _process.mjs {layer} {component}` — one deterministic call that splits into `dark.html` + `light.html`, renders each theme, and gates the result through all three audit axes, returning one JSON verdict. The agent never hand-separates themes or counts `../` link depth.
...
The three governance axes — **token-purity + link-resolution** (`_audit.mjs`) and **render-sanity** (`_sanity.py`) — gate every phase (all three run inside `_process.mjs`). With `--views-from`, **coverage** (`_coverage.mjs`) gates Phase 5. If any artifact fails, the orchestrator re-dispatches that component's subagent with the offending lines quoted. The optional `--fidelity` pass is **advisory and probabilistic** — it files divergence findings, it never gates.
...
SYNTAX:
  ...
  /design-deconstruct <concept-html-path> --doctrine <file> [--doctrine <file> …]
...
[1.5] DETECT EXISTING OUTPUT (re-run UX)
    ...
    IF output directory exists AND contains component folders:
      ...
      IF inventory is non-empty:
        ...
        ASK 1 — Phase selection (AskUserQuestion, multiSelect):
...
[3.5] INVENTORY PRE-SCAN (ONE whole-concept read — kills mid-phase escape churn)
    Dispatch ONE subagent: read the ENTIRE concept (and --tokens-from deck),
    ...
    (AskUserQuestion) before Phase 1. Later phases dispatch FROM this confirmed
...
[6] EXECUTE PHASE 3 — MOLECULES  (T3)
    ...
    FOR EACH molecule (from inventory; sequential default / waves): dispatch per
      docs/PHASE-CONTRACTS.md § 3 — compose atoms by class; never redefine atom
...
[7] EXECUTE PHASE 4 — ORGANISMS  (T4)
    ...
    FOR EACH organism (from inventory): docs/PHASE-CONTRACTS.md § 4. Compose
...
[8] EXECUTE PHASE 5 — VIEWS  (T5)
    ...
    FOR EACH view-mock (route → state leaf):
      ...
      Dispatch subagent (docs/PHASE-CONTRACTS.md § 5) with the inventory key +
...
2. **Every subagent dispatch includes the aesthetic brief** from `Skill("frontend-design:frontend-design", …)`. That skill steers the distinctive look; `design-deconstruct` owns the structural discipline.
...
5. **Delegation boundary.** The orchestrator dispatches exactly the roles this skill names (frontend-designer per component, the pre-scan reader, the fidelity reviewer). Subagents execute directly and never spawn their own helpers. Current-generation models delegate readily — the preamble says so explicitly, and the orchestrator must not "helpfully" add intermediate agents either.
6. **Deterministic vs probabilistic boundary.** Anything that must ALWAYS be correct is orchestrator-owned CODE (the staged tooling), never an agent judgment call:
   ...
   Subagents use `Read/Write/Edit/Glob/Grep` + `frontend-design` consultation only — they author documents and (for tokens) fill data tables; they never shell out to Chrome, separate themes, count link depth, or hand-bundle.
...
8. **Cascade regen is orchestrator-driven** — on VARIANT_REQUEST, dispatch a *targeted* frontend-designer subagent limited to adding the new variant, then `node _process.mjs {layer} {name}` + rebuild bundles.
...
2. Dispatches a targeted subagent to layer N-1 (or N-2/N-3) with the variant spec.
3. Target subagent updates the lower component's README (new variant row) + gallery (new state cell).
...
2. Five TaskList tasks chain via `addBlockedBy` and complete in order.
...
/design-deconstruct .spec/prds/app-v1/designs/concepts.html \
...
/design-deconstruct concepts.html --parallel 4 --fidelity
