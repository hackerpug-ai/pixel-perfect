...
Reverse-engineers a Claude Design exported HTML file into a token-governed atomic design system. Every mock ships **one file per theme** (`dark.html` + `light.html`, each with its own PDF + PNG); views are **state-split** into nested route folders and can be **driven by an external view inventory** with a deterministic coverage gate. Deterministic tooling owns token emission (with a live semantic ⇄ concept round-trip), theme-split, link depth, bundling, render, per-component processing, a Phase-0 concept preflight, and a 3-axis audit (token-purity + link-resolution + render-sanity).
...
├── _process.mjs _audit.mjs _sanity.py _coverage.mjs          # staged tooling
